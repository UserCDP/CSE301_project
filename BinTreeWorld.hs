...
-- a small binary tree
a_tree :: Bin Int
a_tree = B 1 (B 2 (L 2) (B 3 (L 1) (L 1))) (L 2)
  ...
  where
    ...
    go z = do                                          -- give the player some information
      ...
      case parseInput parseCmd line of                 -- parse the input
          ...
          Just Go_Left ->
            case z of
              (c,B x t1 t2) -> go (B0 x c t2,t1)           -- climb up to the left
              (c,L _) -> do
          ...
          Just Go_Right ->
            case z of
              (c,B x t1 t2) -> go (B1 x t1 c,t2)           -- climb up to the right
              (c,L _) -> do
                putStrLn "You cannot climb any further."
                go z
          ...
          Just Go_Down ->
            case z of
              (B0 x c t2,t) -> go (c,B x t t2)             -- climb down from the left, or
              (B1 x t1 c,t) -> go (c,B x t1 t)             -- climb down from the right, or
