...
plug Hole      t = t
plug (B0 x c t2) t = plug c (B x t t2)
plug (B1 x t1 c) t = plug c (B x t1 t)
...
go_left (c,B x t1 t2) = Just (B0 x c t2,t1)  -- focus on the left child
go_left (c,L _)     = Nothing            -- (leaf => no left child)

go_right :: BinZip a -> Maybe (BinZip a)
go_right (c,B x t1 t2) = Just (B1 x t1 c,t2) -- focus on the right child
go_right (c,L _)     = Nothing           -- (leaf => no right child)

go_down :: BinZip a -> Maybe (BinZip a)
go_down (B0 x c t2,t) = Just (c,B x t t2)    -- focus on parent *from* left child
go_down (B1 x t1 c,t) = Just (c,B x t1 t)    -- focus on parent *from* right child
